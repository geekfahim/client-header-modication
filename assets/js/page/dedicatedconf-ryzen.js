...
function updateRamOptions() {
    ...
    const isRyzenProcessor = selectedProcessor.includes('7600X') || 
                             selectedProcessor.includes('7700X') || 
                             selectedProcessor.includes('7900X') || 
                             selectedProcessor.includes('7950X') || 
                             selectedProcessor.includes('7950X3D');
    ...
    for (let i = 0; i < ramOptions.length; i++) {
        ...
        if (option.classList.contains('ram-ddr5')) {
            // Afficher les options DDR5 seulement si c'est un processeur Ryzen spécifié
            option.style.display = isRyzenProcessor ? 'block' : 'none';
        } else if (option.textContent.includes('DDR4')) {
            // Afficher les options DDR4 dans les autres cas
            option.style.display = isRyzenProcessor ? 'none' : 'block';
        }
    }
}

// Ajouter un écouteur d'événements sur le menu déroulant du processeur
processeur.addEventListener('change', updateRamOptions);
